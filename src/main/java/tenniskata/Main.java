...
class TennisGame {
    ...
    public String playTennis() {
        ...
        {
            score = over4pointCalc();
        }
        else
        {
            score = generalCalc(score);
        }
        return score;
    }

    private String generalCalc(String score) {
        int tempScore;
        for (int i = 1; i<3; i++)
        {
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 33; i++) {
            ...
            for(int p1Cnt = 0 ; p1Cnt < (Integer)testCase[i][0]; p1Cnt ++) {
                tennisGame.getPoint("player1");
            }
            for(int p2Cnt = 0 ; p2Cnt < (Integer)testCase[i][1]; p2Cnt ++) {
                tennisGame.getPoint("player2");
            }
