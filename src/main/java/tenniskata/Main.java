...
class TennisGame {
    ...
    public String getScore() {
        ...
        {
            return sameScoreName(player1Score);
        }
        else if (player1Score >= MIN_SCORE_FOR_ADVANTAGE || player2Score >= MIN_SCORE_FOR_ADVANTAGE)
        {
            int scoreDifference = player1Score - player2Score;
            return winnerScore(scoreDifference);
        }
        return getScoreName(player1Score) + "-" + getScoreName(player2Score);
    }

    private static String winnerScore(int scoreDifference) {
